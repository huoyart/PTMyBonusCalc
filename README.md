...
除默认 NexusPHP 公式外，当前已加入以下分站公式处理：

- `audiences.me`：支持官组/普通种子权重、零魔规则，以及普通/官组不同 B0 的参数页结构；种子列表显示带权重后的 A 值。
- `hdarea.club`：支持新版 v2.0 算法的单种预估分，包含数量分和体积稀缺分；显示值未扣除全局 100/900 上限。
- `hhanclub.net`：当前魔力页未提供完整基础公式参数，脚本不会强行套用默认公式，避免显示错误结果。
